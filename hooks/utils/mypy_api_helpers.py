...
def get_exclude_dirs_from_config(
    section_name: str = 'flake8',
    param_name: str = 'exclude',
) -> List[str]:
    exclude_param = get_param_from_configs(section_name, param_name)
    return [] if exclude_param is None else [item.strip() for item in exclude_param.split(',') if item.strip()]
...
def get_list_param_from_config(
    ...
    raw_value = get_param_from_config(
        config_file_name, section_name, param_name)
    return [] if raw_value is None else [value.strip() for value in raw_value.split('\n') if value.strip()]
