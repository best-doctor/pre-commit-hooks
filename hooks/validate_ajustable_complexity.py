...
def main() -> Optional[int]:
    ...
    default_max_allowed_complexity = int(get_param_from_configs(
        'flake8',
    ...
    for pyfilepath in get_input_files():
        ast_tree_results: Tuple[Optional[ast.AST],
                                Optional[str]] = get_ast_tree_with_content(pyfilepath)
        ...
        for funcdef in funcdefs:
            ...
            blacklisted_vars_amount = (
                ...
                + len([v for v in all_vars_in_function if len(v)
                      == 1 and v not in ['_']])
            )
            ...
            max_complexity = current_max_complexity - \
                blacklisted_vars_amount * complexity_penalty
            ...
            if current_complexity > max_complexity and '# noqa' not in def_line:
                errors.append(
                    (pyfilepath, funcdef.lineno, funcdef.name,
